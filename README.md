...
| Статус | Описание |
| ------ | -------- |
| **total** | информация о том, сколько свободно в системе за минусом то, что занимает Ядро |
| **used** | сколько памяти потребляют приложения |
| **free** | объем памяти, который на данный момент системе не нужен, то есть можно вытащить из системы |
| **shared** | разделяемая память, память для межпроцессорного взаимодействия |
| **buff/cache** | буфер и кэш |
| **available** | сколько памяти будет доступно при необходимости |
